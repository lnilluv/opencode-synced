...
import { canCommitMcpSecrets, hasSecretsBackend, loadOverrides, loadState, loadSyncConfig, normalizeSyncConfig, updateState, writeSyncConfig, } from './config.js';
...
import { computeSecretsHash, createSecretsBackend, resolveRepoAuthPaths, resolveSecretsBackendConfig, } from './secrets-backend.js';
...
export function createSyncService(ctx) {
    ...
    const resolveSecretsBackend = (config) => {
    ...
    const ensureAuthFilesNotTracked = async (repoRoot, config) => {
        if (!hasSecretsBackend(config))
            return;
        ...
        throw new SyncCommandError(`Sync repo already tracks secret auth files (${trackedList}). ` +
            'Remove them and rewrite history before enabling a secrets backend.');
    };
    const computeSecretsHashSafe = async () => {
        try {
    ...
    const runSecretsPullIfConfigured = async (config) => {
    ...
    const runSecretsPushIfConfigured = async (config) => {
        const backend = resolveSecretsBackend(config);
        if (!backend)
            return 'not_configured';
        return await pushSecretsWithBackend(backend);
    };
    const secretsBackendNotConfiguredMessage = 'Secrets backend not configured. Add secretsBackend to opencode-synced.jsonc.';
    const resolveSecretsBackendForCommand = async () => {
    ...
    const runSecretsCommand = async (action) => {
    ...
    return {
        ...
        secretsPull: () => runExclusive(() => runSecretsCommand(async (backend) => {
            await backend.pull();
            await updateSecretsHashState();
            return 'Pulled secrets from 1Password.';
        })),
        secretsPush: () => runExclusive(() => runSecretsCommand(async (backend) => {
        ...
        secretsStatus: () => runExclusive(() => runSecretsCommand(async (backend) => {
...
function assertValidSecretsBackend(config) {
    const resolution = resolveSecretsBackendConfig(config);
    if (resolution.state === 'invalid') {
        throw new SyncCommandError(resolution.error);
    }
}
async function isRepoPathTracked($, repoRoot, repoRelativePath) {
...
async function runStartup(ctx, locations, config, log, options) {
    const repoRoot = resolveRepoRoot(config, locations);
